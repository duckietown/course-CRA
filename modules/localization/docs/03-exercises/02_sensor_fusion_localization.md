...
* A screen recording similar to this [](#encoder-exercise-example) where you drive the robot in a loop and try to "close the loop" (by driving to the point where you started from). Make sure that the video contains the camera images as well as the TF tree in Rviz. Use a visible landmark as the origin of the map frame.
...
* [This link](http://docs.ros.org/en/jade/api/tf/html/python/transformations.html) contains many useful methods from the `tf` library that allow you to switch between representations (e.g. expressing an euler yaw angle as a quaternion).
...
To calculate the static transform between the `map` and `apriltag` frames ($$ T_{MA} $$) use [](#fig:traffic-sign-at) as reference. To calculate the static transform between the `baselink` and `camera` frames, you can use 
...
* A screen recording similar to this [](#fused-exercise-example-rectified-slow),  where you drive the robot in a loop around the Apriltag with the virtual joystick. Start the recording with the Apriltag not visible, so that you validate that your calibration service is working and when it does bacome visible all the frames align. You should end the trajectory where you started it (feel free to use a marker on the ground). Make sure that the video contains the camera images as well as the TF tree in Rviz.
<!-- 
